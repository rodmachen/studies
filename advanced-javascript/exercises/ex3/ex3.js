...
NotesManager.prototype.init = function(opts) {
		...
		this.$open_help.bind("click",handleOpenHelp);

		// listen to "add" button
		this.$add_note.bind("click",handleAddNote);

		// listen for <enter> in text box
		this.$new_note.bind("keypress",handleEnter);

		// listen for clicks outside the notes box
		$(document).bind("click",handleDocumentClick);

		// listen for clicks on note elements
		this.$notes.on("click",".note",handleNoteClick);
	};
